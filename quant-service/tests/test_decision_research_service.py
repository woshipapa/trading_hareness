...
class DecisionResearchServiceTests(unittest.TestCase):
    def test_complete_market_structure_candidate_closes_all_gates_and_is_executable(self):
        dossier = build_dossier(candidate_evidence(), as_of_date=date(2026, 9, 1), holding=False)
        self.assertEqual(dossier.status, "passed")
        self.assertEqual([gate.gate_key for gate in dossier.gates], [f"G{index}" for index in range(8)])
        g6 = next(gate for gate in dossier.gates if gate.gate_key == "G6")
        self.assertEqual(g6.verdict, "pass")
        self.assertTrue(g6.independent_run)
        self.assertEqual(g6.evidence["independence_boundary"], "bullish score and candidate rank were not inputs")
        self.assertGreater(dossier.evidence_snapshot["geometry"]["reward_risk_1"], 1)


    def test_negative_board_flow_is_a_terminal_rejection_not_an_unfinished_placeholder(self):
        ...
        self.assertEqual(next(gate for gate in dossier.gates if gate.gate_key == "G3").verdict, "fail")
        self.assertTrue(all(gate.verdict != "unknown" for gate in dossier.gates))


    def test_independent_downside_gate_does_not_accept_missing_geometry(self):
        gate = independent_downside_gate(None, [])
        self.assertEqual(gate.verdict, "unknown")
        self.assertTrue(gate.independent_run)


    def test_passed_contract_rejects_non_independent_g6(self):
        gates = [ResearchGate(
    ...
    def test_holding_plan_warning_precedes_stop_and_cap_is_a_policy_limit(self):
        evidence = candidate_evidence()
        evidence["bars"] = [
        ...
        self.assertLess(float(plan.stop_price), warning)
        self.assertLess(warning, close)
        self.assertEqual(plan.max_position_pct, Decimal("25"))
        self.assertEqual(plan.metadata["current_position_weight_pct"], 81.5)
